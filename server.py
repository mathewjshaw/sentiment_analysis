...
def add_color(tweets):
    ...
    for t in tweets:
        ...
        t['color'] = colors[scaled_score]


@app.route("/<name>")
def tweets(name):
    "Display the tweets for a screen name color-coded by sentiment score"
    user_dictionary = fetch_tweets(api, name)
    add_color(user_dictionary['tweets'])
    all_sentiments = [t['score'] for t in user_dictionary['tweets']]
    median_sentiment = median(all_sentiments)
    return render_template("tweets.html", tweets = user_dictionary['tweets'], name = name, median_sentiment = median_sentiment)
...
def following(name):
    ...
    """
    users_followed = fetch_following(api, name)
    return render_template("following.html", users = users_followed, name = name)
